...
class CleaningRecommendationController extends GetxController {
  final CleaningRepository _repository = CleaningRepository();
  final ImagePicker _picker = ImagePicker();
  final FirebaseAuth _auth = FirebaseAuth.instance;

  // List State
  final RxList<CleaningRecommendation> recommendations = <CleaningRecommendation>[].obs;
  ...
  final addressController = TextEditingController(); // For neighborhood context

  @override
  void onInit() {
    super.onInit();
    _bindRecommendations();
  }
  ...
  void _bindRecommendations() {
    isLoading.value = true;
    _repository.getRecommendations().listen((list) {
  ...
  Future<void> createRecommendation() async {
    ...
    try {
      ...
      if (selectedImage.value != null) {
        imageUrl = await _repository.uploadImage(selectedImage.value!, 'recommendation');
      }

      final newRecommendation = CleaningRecommendation(
      ...
      await _repository.createRecommendation(newRecommendation);
      Get.back(); // Close write page
      Get.snackbar('성공', '추천글이 등록되었습니다.');
      clearWriteForm();
    } catch (e) {
